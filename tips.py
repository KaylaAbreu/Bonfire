...
class AllTips(Screen):
    ...
    def expand_tip(self, title, tip):
        ...
        # body_layout = MDBoxLayout(orientation="vertical")
        # body_layout.add_widget(MDLabel(text=tip_content))

        # body = ScrollView()
        # body.add_widget(body_layout)

        # text = tip_content
        # content = body
        dialog = MDDialog(title=title, text=tip_content)
        dialog.open()
...
class MountainTips(Screen):
    def on_enter(self):
        app = MDApp.get_running_app()
        app.cursor.execute("SELECT * FROM tips WHERE category = 'Mountains'")
        tips = app.cursor.fetchall()
